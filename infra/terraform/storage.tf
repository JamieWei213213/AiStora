resource "aws_s3_bucket" "datasets" {
  bucket        = "${local.name}-${data.aws_caller_identity.current.account_id}-${var.aws_region}-datasets"
  force_destroy = var.dataset_force_destroy

  tags = { Name = "${local.name}-datasets" }
}
...
data "aws_iam_policy_document" "dataset_bucket" {
  statement {
...
resource "aws_s3_bucket_policy" "datasets" {
  bucket = aws_s3_bucket.datasets.id
  policy = data.aws_iam_policy_document.dataset_bucket.json
